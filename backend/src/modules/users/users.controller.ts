...
/* ============================================================
   UPDATE STUDENT / ORGANIZER STATUS
============================================================ */
...
export async function adminListOrganizers(
  req: Request,
  res: Response
) {
  const search = String(
...
export async function adminGetOrganizer(
  req: Request,
  res: Response
) {
  const id = Number(
...
const adminAccountUpdateSchema =
  z.object({
    name: z
      .string()
      .min(2)
...
export async function adminUpdateAccount(
  ...
  const input =
    adminAccountUpdateSchema.parse(
      req.body
    );
...
export async function adminDeleteAccount(
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      await tx.bookBorrow.deleteMany(
        {
          where: {
            userId: id,
          },
  ...
  /*
   * Delete profile image from
   * Cloudinary after DB deletion.
   */
  await deleteLocalProfileAvatar(
    current.avatarUrl
  );
